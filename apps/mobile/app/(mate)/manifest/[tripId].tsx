...
export default function ManifestScreen() {
  ...
  const syncAndRefresh = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/mate/manifest?tripId=${tripId}`, {
  ...
  const performCheckIn = useCallback(
    async (ticket: MateTicket, booking: MateBooking, method: "qr" | "name_search" | "manual") => {
      if (!tripId) return;
  ...
  const totalTickets =
    manifest?.bookings.reduce((n, b) => n + b.tickets.filter((t) => !t.voided).length, 0) ?? 0;
  const totalChecked =
    manifest?.bookings.reduce(
      (n, b) =>
        n + b.tickets.filter((t) => !t.voided && (t.checkedIn || localCheckedIn.has(t.id))).length,
      0,
    ) ?? 0;
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <View style={[s.tripBar, { borderLeftColor: manifest.trip.vessel.color }]}>
        ...
        {(manifest.trip.status === "sailed" || manifest.trip.status === "pending_settlement") && (
          <TouchableOpacity
            ...
            onPress={() =>
              router.push({
                pathname: "/(mate)/report/[tripId]" as any,
        ...
        <View style={s.capacityBar}>
          ...
          <View style={s.capacityBtns}>
            <TouchableOpacity
              style={[
                s.capBtn,
                (manifest.trip.capacity <= totalTickets || capacityUpdating) && s.capBtnDisabled,
              ]}
            ...
            <TouchableOpacity
              style={[
                s.capBtn,
                (manifest.trip.capacity >= manifest.trip.vessel.certificateCapacity ||
                  capacityUpdating) &&
                  s.capBtnDisabled,
              ]}
              onPress={() => adjustCapacity(1)}
      ...
      {/* ── Camera scan modal ── */}
      <Modal visible={scanMode === "camera"} animationType="slide" onRequestClose={closeScanMode}>
