...
- [x] **Middleware trusts client-supplied `x-operator-id` header**
...
- [x] **`vercel.json` crons array is empty — no crons are actually running**
...
- [x] **Demo reset has no operator scoping — wipes all operators' data**
...
- [x] **`clientIp()` reads the leftmost (attacker-controlled) `x-forwarded-for` value**
...
- [x] **Cross-operator trip booking via unsanitized cart `tripId`**
...
- [x] **Stripe Connect OAuth has no `state` parameter (CSRF)**
...
- [x] **`extend-hold` has no auth, no operator scope, no extension cap**
...
- [x] **ISR cache on fishing-reports pages serves one operator's data to all operators**
...
- [x] **Boarding, confirmation, and delivery pages have no operator scope**
...
- [x] **`GET /api/bookings` wallet lookup has no operator scope**
...
- [x] **Push register uses the spoofable `x-operator-id` header instead of token's `operatorId`**
...
- [x] **Staff lookup at mate and admin login is not scoped by operator**
...
- [x] **Admin capacity PATCH is a read-modify-write race (can overbook)**
...
- [x] **Mate capacity PATCH has the same read-modify-write race**
  `apps/web/src/app/api/mate/trips/[tripId]/capacity/route.ts:20-80`
  Fix: same as above.

- [x] **`POST /api/bookings` does not check `trip.status`**
...
- [x] **No DB-level `CHECK (seats_remaining >= 0)` constraint**
...
- [x] **`expire-pending-bookings` will time out on large backlogs**
...
- [x] **`/api/platform/auth` has no rate limit**
...
- [x] **Token vs. request header operator mismatch in `requireCustomer`/`requireMate`**
...
- [x] **`trip-reminders` has N+1 queries per trip — timeout risk at scale**
...
- [x] **Stripe-PI-failure rollback path is not idempotent with the expiry cron**
...
- [x] **Admin trip cancellation resets `seatsRemaining = capacity` from a stale read**
...
- [x] **Email not normalized to lowercase in `POST /api/bookings`**
...
- [x] **Rate-limit keys don't include `operatorId` — cross-tenant DoS**
...
- [x] **Old OTPs are not invalidated when a new one is requested**
...
- [x] **Timing oracle on mate and admin login**
...
- [x] **Wallet-lookup IP-bucket reset enables targeted enumeration**
...
- [x] **CRON_SECRET comparison is not timing-safe on all three cron routes**
...
- [x] **`trip-reminders` sends duplicate push notifications per email**
...
- [x] **Duplicate `tripId` in cart silently creates two `bookingItems` for one trip**
...
- [x] **Hostname not lowercased in middleware domain lookup**
...
- [x] **Verify `push.ts` scopes push-token lookup by `operatorId`**
