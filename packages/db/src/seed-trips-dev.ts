...
async function run() {
  ...
  for (const vessel of vessels) {
    ...
    const products = await db
      ...
      .where(and(
        eq(schema.products.vesselId, vessel.id),
    ...
    const product = products.find((p) =>
      p.category !== "Fireworks" && prices.some((pr) => pr.productId === p.id)
    );
    if (!product) {
    ...
    console.log(`  ✓ ${vessel.name}: "${product.displayName}" — ${tripRows.length} trips (${dates[0]} → ${dates[dates.length - 1]})`);
  }
